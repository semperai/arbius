...
async function processContestationVoteFinish(taskid: string) {
  ...
  if (Math.random() < 1 - c.prob.contestation_vote_finish) {
    log.debug(
      `[processContestationVoteFinish] ContestationVoteFinish ${taskid} skipped`
    );
    return;
  }
...
async function processValidatorStake() {
  ...
  log.debug(
    `[processValidatorStake] Ether balance: ${ethers.utils.formatEther(
      etherBalance
    )}`
  );
  ...
  log.debug(
    `[processValidatorStake] AIUS Staked: ${ethers.utils.formatEther(staked)}`
  );
  ...
  log.debug(
    `[processValidatorStake] Validator Minimum: ${ethers.utils.formatEther(
      validatorMinimum
    )}`
  );
  ...
  log.debug(
    `[processValidatorStake] Deposit Amount ${ethers.utils.formatEther(
      depositAmount
    )}`
  );
  ...
  if (balance.lt(depositAmount)) {
    log.error(
      `[processValidatorStake] Balance ${ethers.utils.formatEther(
        balance
  ...
  log.debug(
    `[processValidatorStake] Allowance Amount ${ethers.utils.formatEther(
      allowance
    )}`
  );
  ...
  if (allowance.lt(balance)) {
    ...
    await expretry(async () => {
      ...
      log.info(
        `[processValidatorStake] Allowance increased in ${receipt.transactionHash}`
      );
    });
  }

  log.debug(
    `[processValidatorStake] Depositing for validator stake ${ethers.utils.formatEther(
      depositAmount
    )}`
  );
  await expretry(async () => {
  ...
  log.debug(
    `[processValidatorStake] Post staked: ${ethers.utils.formatEther(
      postDepositStaked
    )}`
  );
}
...
async function processTask(taskid: string, txid: string) {
  try {
    ...
    if (!modelEnabled) {
      log.debug(
        `[processTask] Task (${taskid}) is using non-enabled Model (${model})`
      );
      return;
    }
    ...
    let input = await lookupAndInsertTaskInput(
      taskid,
      inputCid,
      txid,
      modelTemplate
    ...
    log.debug(
      `[processTask] Task (${taskid}) input ${JSON.stringify(input, null, 2)}`
    );

    const m = getModelById(EnabledModels, model);
    if (m === null) {
      log.error(
        `[processTask] Task (${taskid}) could not find model (${model})`
      );
      return;
    }
    ...
    async function checkForExistingSolution() {
      if (alreadySeenSolution.has(taskid)) {
        ...
        log.info(
          `[processTask] Solution found with cid ${existingSolutionCid} does not match ours ${solutionCid}`
        );
    ...
    try {
      if (c.read_only) {
        log.info(
          `[processTask] Read only mode, not signalling commitment for ${taskid}`
        );
      } else {
      ...
      log.error(
        `[processTask] Commitment submission failed ${JSON.stringify(e)}`
      );
      return;
    }
    ...
    await expretry(
      async () => {
        try {
          log.debug(
            `[processTask] Submitting solution ${taskid} ${solutionCid}`
          );
          if (c.read_only) {
            log.info(
              `[processTask] Read only mode, not submitting solution for ${taskid}`
            );
          } else {
            ...
            const receipt = await tx.wait();
            log.info(
              `[processTask] Solution submitted in ${receipt.transactionHash}`
            );
          }
          ...
          if (await checkForExistingSolution()) return;
          log.error(
            `[processTask] Solution submission failed ${JSON.stringify(e)}`
          );
        }
      },
      2,
...
async function processSolution(taskid: string) {
  try {
    ...
    if (!m) {
      log.error(
        `[processSolution] Task (${taskid}) could not find model (${model})`
      );
      return;
    }
    ...
    if (solution.cid !== cid) {
      log.info(
        `[processSolution] Solution found with cid ${solution.cid} does not match ours ${cid}`
      );
      await contestSolution(taskid);
    } else {
      log.info(
        `[processSolution] Solution CID matches our local CID for ${taskid}`
      );
    }
...
async function processContestation(validator: string, taskid: string) {
  try {
    ...
    if (!canVote) {
      log.error(
        `[processContestation] Contestation ${taskid} cannot vote (code ${canVoteStatus})`
      );
      return;
    }
    ...
    if (m === null) {
      log.error(
        `[processContestation] Task (${taskid}) could not find model (${model})`
      );
      return;
    }
    ...
    if (invalidTask != null) {
      log.info(
        `[processContestation] Contested task ${taskid} has invalid input. Voting in favor.`
      );
      ...
      if (!taskInput) {
        log.error(
          `[processContestation] Task (${taskid}) input not found in db`
        );
        return;
      }
      const input = JSON.parse(taskInput.data);
      ...
      if (solution.cid !== expectedCid) {
        log.info(
          `[processContestation] Contested CID does not match expected CID for ${taskid}. Voting true`
        );
        await voteOnContestation(taskid, true);
      } else {
        log.info(
          `[processContestation] Contested CID matches expected CID for ${taskid}. Voting false.`
        );
...
async function contestSolution(taskid: string) {
  try {
    ...
    if (validatorStake.lt(validatorMinimum.mul(110).div(100))) {
      log.info(
        '[contestSolution] Validator stake is less than 110% of minimum, not contesting'
      );
      return;
    }

    const { validator } = await expretry(
      async () => await arbius.solutions(taskid)
    );
    ...
    if (validator === wallet.address) {
      log.error(
        `[contestSolution] Attempting to contest own solution ${taskid}  --- lets not do this`
      );
    ...
    if (c.read_only) {
      ...
      const receipt = await tx.wait();
      log.info(
        `[contestSolution] Submitted contestation for ${taskid} in ${receipt.transactionHash}`
      );
      alreadySeenContestationTask.add(taskid);
    ...
    if (
      ...
      log.error(
        `[contestSolution] An unknown error occurred when we tried to contest ${taskid}`
      );
      return;
    }

    log.info(
      `[contestSolution] Contestation for ${taskid} was already created by ${existingContestationValidator}`
    );
...
async function voteOnContestation(taskid: string, yea: boolean) {
  ...
  if (validatorStake.lt(validatorMinimum.mul(110).div(100))) {
    log.info(
      '[voteOnContestation] Validator stake is less than 110% of minimum, not voting'
    );
    return;
  }
  ...
  try {
    if (c.read_only) {
      log.info(
        `[voteOnContestation] Read only mode, not voting ${
          yea ? 'YES' : 'NO'
        } on contestation ${taskid}`
      );
    } else {
      log.info(
        `[voteOnContestation] Attempt to vote ${
          yea ? 'YES' : 'NO'
        } on ${taskid} contestation`
      );
      const tx = await solver.voteOnContestation(taskid, yea);
      const receipt = await tx.wait();
      log.info(
        `[voteOnContestation] Contestation vote ${
          yea ? 'YES' : 'NO'
        } submitted on ${taskid} in ${receipt.transactionHash}`
      );
    }
  } catch (e) {
    log.debug(JSON.stringify(e));
  ...
  try {
    const receipt = await expretry(
      async () => {
        ...
        log.debug(
          '[processClaim] contestationValidator',
          contestationValidator
        );
        if (
          contestationValidator != '0x0000000000000000000000000000000000000000'
        ) {
          log.error(
            `[processClaim] Contestation found for solution ${taskid}, cannot claim`
          );
        ...
        if (c.read_only) {
          ...
          const receipt = await tx.wait();
          log.info(
            `[processClaim] Claim ${taskid} in ${receipt.transactionHash}`
          );
          return receipt;
        }
      },
