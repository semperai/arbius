...
<Note>Make sure you restart container after testing and test again.</Note>
...
<Note>**New in V6**: You can register models with allow lists to control who can submit solutions. This is perfect for quality control during launch phases or private deployments.</Note>

### What are Allow Lists?

Allow lists let you restrict which validators can submit solutions for your model. This provides:

- **Quality assurance** during model launch phases
- **Private deployments** for enterprise or sensitive models
- **Gradual rollouts** from trusted miners to permissionless
- **Controlled solver network** for model token launches (Amica Personas)

### When to Use Allow Lists

**Use allow lists when:**
- Launching a new model and want to test with trusted miners first
- Building a private/enterprise model that shouldn't be public
- Creating a model token and need controlled quality during bonding curve phase
- Model requires specific hardware/setup that only certain miners have

**Don't use allow lists if:**
- You want maximum decentralization and permissionless access
- Your model is already well-tested and production-ready
- You're comfortable with any validator attempting to solve tasks

### Allow List Lifecycle

1. **Register model with allow list** - Start with restricted access
2. **Add/remove validators** as needed - Manage permissions dynamically
3. **Open to all** (optional) - Disable allow list when ready for permissionless operation

<Warning>
**Important**: Once you disable an allow list with `disableModelAllowList()`, it **cannot be re-enabled**. This is a security feature to prevent "rug pulls" where access is restricted after being opened.
</Warning>
...
<Note>Your private key should not start with `0x` if it does you can just remove it as "0x" is just representation to indicate the hexadecimal format.</Note>
...
#### Option 1: Register without allow list (permissionless)
...
#### Option 2: Register with allow list (V6+)

To register a model with an allow list of approved solvers:

```bash
npx hardhat --network arbitrum engine:registerModelWithAllowList \
--address "0x0000000000000000000000000000000000000001" \
--fee "0" \
--template templates/[your-template-file].json \
--allowlist "0xValidator1Address,0xValidator2Address,0xValidator3Address"
```

<Note>
Separate validator addresses with commas (no spaces). Only these addresses will be able to submit solutions initially.
</Note>

#### Registration Output

Either command will result in a line like:
...
### Managing Allow Lists (V6+)

After registering with an allow list, you can manage it using these commands:

#### Add validators to allow list

```bash
npx hardhat --network arbitrum engine:addToModelAllowList \
--model "0x873de8267de30d5650091ef5467f682b3d2736522ed58ee3fc40f1244941feaf" \
--solvers "0xNewValidator1,0xNewValidator2"
```

#### Remove validators from allow list

```bash
npx hardhat --network arbitrum engine:removeFromModelAllowList \
--model "0x873de8267de30d5650091ef5467f682b3d2736522ed58ee3fc40f1244941feaf" \
--solvers "0xOldValidator1,0xOldValidator2"
```

#### Disable allow list (open to all validators)

```bash
npx hardhat --network arbitrum engine:disableModelAllowList \
--model "0x873de8267de30d5650091ef5467f682b3d2736522ed58ee3fc40f1244941feaf"
```

<Warning>
**This action is irreversible!** Once disabled, the allow list cannot be re-enabled. Only use this when you're ready to make your model permanently permissionless.
</Warning>

#### Check allow list status

To check if a validator can submit solutions:

```bash
npx hardhat --network arbitrum engine:isSolverAllowed \
--model "0x873de8267de30d5650091ef5467f682b3d2736522ed58ee3fc40f1244941feaf" \
--solver "0xValidatorAddress"
```

To check if a model requires an allow list:

```bash
npx hardhat --network arbitrum engine:modelRequiresAllowList \
--model "0x873de8267de30d5650091ef5467f682b3d2736522ed58ee3fc40f1244941feaf"
```
