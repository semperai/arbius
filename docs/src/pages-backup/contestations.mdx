...
<Note>
  **V6 introduces master contesters** - a curated group of trusted validators who can initiate contestations. This provides an additional security layer for the network.
</Note>
...
<Row>
  <Col>
    <Note>
      **New in V6**: Anyone can suggest a contestation to alert master contesters of a potentially invalid solution.
    </Note>
...
<Row>
  <Col>
    <Note>
      **V6 Change**: Only master contesters can submit contestations. Regular validators should use `suggestContestation()` instead.
    </Note>
