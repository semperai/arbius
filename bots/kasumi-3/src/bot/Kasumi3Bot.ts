...
import { TIMEOUTS, RATE_LIMITS, JOB_QUEUE, HEALTH_CHECK, REWARDS, STARTUP, ARBIUS } from '../constants';

/**
 * Kasumi-3 Bot - Multi-model Telegram bot for Arbius network
 */
export class Kasumi3Bot {
  ...
  constructor(
    ...
    this.rateLimiter = new RateLimiter(
      rateLimitConfig || {
        maxRequests: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS || String(RATE_LIMITS.MAX_REQUESTS_DEFAULT)),
        windowMs: parseInt(process.env.RATE_LIMIT_WINDOW_MS || String(RATE_LIMITS.WINDOW_MS_DEFAULT))
      }
    );
  ...
  private setupHandlers(): void {
    ...
    this.bot.use(async (ctx, next) => {
      ...
      if (!this.rateLimiter.checkLimit(userId)) {
        ...
        await ctx.reply(
          ...
          `Limit: ${RATE_LIMITS.REQUESTS_PER_MINUTE_TEXT}`
        );
        return;
      }
    ...
    this.bot.on(message('text'), async ctx => {
      if (now() - this.startupTime < STARTUP.IGNORE_MESSAGES_SECONDS) {
  ...
  private async handleModelCommand(ctx: Context, modelConfig: ModelConfig, prompt: string): Promise<void> {
    if (!prompt) {
      await ctx.reply(`Please provide a prompt. Usage: /${modelConfig.name} <your prompt>`);
      return;
    }
    ...
    try {
      ...
      const taskUrl = `${ARBIUS.TASK_URL_BASE}/${taskid}`;
  ...
  private async handleStatus(ctx: Context): Promise<void> {
    try {
      ...
      const hasEnoughGas = ethBalance > ethers.parseEther(HEALTH_CHECK.MIN_ETH_BALANCE);
      const hasEnoughAius = arbiusBalance > ethers.parseEther(HEALTH_CHECK.MIN_AIUS_BALANCE);
      const isStakedEnough = validatorStaked >= validatorMinimum;
      const queueHealthy = queueStats.processing < HEALTH_CHECK.QUEUE_HEALTHY_THRESHOLD;
  ...
  private async handleProcess(ctx: Context): Promise<void> {
    if (!ctx.message || !('text' in ctx.message)) {
      return;
    }
    const parts = ctx.message.text.split(' ');
  ...
  private async waitForJobCompletion(job: TaskJob, ctx: Context, responseCtx?: any): Promise<void> {
    const maxWaitTime = parseInt(process.env.JOB_WAIT_TIMEOUT_MS || String(TIMEOUTS.JOB_WAIT_DEFAULT));
    let lastProgress = '';
  ...
  private async sendCompletedResult(ctx: Context, responseCtx: any, job: TaskJob): Promise<void> {
    ...
    try {
      ...
      if (job.wonReward) {
        const winnerImageUrl = process.env.WINNER_IMAGE_URL || REWARDS.DEFAULT_IMAGE_URL;
        const rewardAmount = process.env.REWARD_AMOUNT || REWARDS.AMOUNT_DEFAULT;
        try {
  ...
  async launch(): Promise<void> {
    ...
    this.cleanupInterval = setInterval(() => {
      this.jobQueue.clearOldJobs(JOB_QUEUE.OLD_JOB_THRESHOLD_MS);
