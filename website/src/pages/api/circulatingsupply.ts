...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const oneProvider = new ethers.providers.JsonRpcProvider('https://arbitrum.llamarpc.com');
  ...
  const oneContract = new ethers.Contract(Config.v4_baseTokenAddress, BaseTokenArtifact.abi, oneProvider);

  const engineBalanceNova = await novaContract.balanceOf(Config.v2_engineAddress);
  const engineBalanceOne = await oneContract.balanceOf(Config.v4_engineAddress);
  const converterBalanceEth = await ethContract.balanceOf(Config.l1OneToOneAddress);
  const converterBalanceNova = await novaContract.balanceOf(Config.l2OneToOneAddress);
  const daoBalanceEth = await ethContract.balanceOf("0xF20D0ebD8223DfF22cFAf05F0549021525015577");
  const daoBalanceOne = await oneContract.balanceOf("0xF20D0ebD8223DfF22cFAf05F0549021525015577");
